...
\newcommand{\learninggoals}{
  \item Momentum-based and Nesterov-accelerated SGD
  \item Learning rate schedules and adaptive learning-rate methods (Adagrad, RMSProp, Adam)
  \item Batch normalization and its effect on the optimization landscape
  \item Hyperparameter tuning: the search problem, grid/random search, Bayesian optimization, multi-fidelity methods, and the untouched-test-set principle
  \item Neural Architecture Search: what it automates and why, illustrated with one concrete example (DARTS)
}
...
\begin{frame} {Momentum: Illustration}
  ...
  \begin{figure}
    \centering
      \scalebox{0.65}{\includegraphics{figure/moment_vis2.png}}
...
\begin{frame} {Learning Rate}
  \begin{itemize}
    ...
    \begin{figure}
    \centering
      \scalebox{0.5}{\includegraphics{figure/lr_jh.png}}
...
\captionsetup{font=scriptsize,labelfont=scriptsize, labelfont = bf}
    \begin{center}
      \href{https://giphy.com/embed/SJVFO3IcVC0M0}{\includegraphics[width = .40\textwidth]{figure/sattle_point2-50.png}}
      \href{https://giphy.com/embed/SJVFO3IcVC0M0}{\includegraphics[width = .40\textwidth]{figure/sattle_point2-150.png}}
...
\begin{frame} {Batch Normalization: Illustration}
  ...
  \end{itemize}
    \begin{figure}
    \centering
      \scalebox{0.26}{\includegraphics{figure/batchy.png}}
...
\begin{frame} {Batch Normalization}
  ...
  \begin{figure}
    \centering
      \scalebox{0.48}{\includegraphics{figure/bn_effect.png}}
  \end{figure}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\bibitem[Ian Goodfellow et al., 2016]{adv1} Ian Goodfellow, Yoshua Bengio and Aaron Courville (2016)
\newblock Deep Learning
\newblock \emph{\url{http://www.deeplearningbook.org/}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\bibitem[Yann Dauphin et al., 2014]{adv2} Yann Dauphin, Razvan Pascanu, {\c{C}}aglar G{\"{u}}l{\c{c}}ehre, Kyunghyun Cho, Surya Ganguli, Yoshua Bengio (2014)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\bibitem[Hao Li et al., 2017]{adv3} Hao Li, Zheng Xu, Gavin Taylor, Christoph Studer, Tom Goldstein (2017)
\newblock Visualizing the Loss Landscape of Neural Nets
\newblock \emph{\url{https://arxiv.org/abs/1712.09913}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\bibitem[Tim Dettmers, 2015]{adv4} Tim Dettmers (2015)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\bibitem[Hafidz Zulkifli, 2018]{adv5} Hafidz Zulkifli (2018)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\bibitem[Loshchilov, Hutterer, 2016]{adv6} Ilya Loshchilov, Frank Hutter (2016)
\newblock SGDR: Stochastic Gradient Descent with Warm Restarts
\newblock \emph{\url{https://arxiv.org/abs/1608.03983}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\bibitem[Jeremy Jordan, 2018]{adv7} Jeremy Jordan (2018)
\newblock Setting the learning rate of your neural network
\newblock \emph{\url{https://www.jeremyjordan.me/nn-learning-rate/}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\bibitem[Santurkar et al. 2018]{adv8} Shibani Santurkar, Dimitris Tsipras, Andrew Ilyas, Aleksander Madry (2018)
\newblock How Does Batch Normalization Help Optimization?
\newblock \emph{\url{https://arxiv.org/abs/1805.11604}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\bibitem[Akshay Chandra 2015]{adv9} Akshay Chandra (2015) 
...
\begin{vbframe}{What Is a Hyperparameter?}
\begin{itemize}
\item \textbf{Tuning} is the process of selecting the best hyperparameters
$\lamv$ for a learner. Hyperparameters are parameters of the learner
itself (e.g. learning rate, batch size, number of hidden layers), as
opposed to the model parameters $\thetav$ found by training.
\item Guitar analogy: hyperparameters are like the tuning pegs; finding the
best $\thetav$ (playing the guitar) is a separate process that depends on
first being in tune.
\item Just as a badly-tuned guitar won't perform well, poorly chosen
hyperparameters can drastically hurt model performance. Tuning helps find
a balance between under- and overfitting.
\item Key training hyperparameters for a neural network include the
learning rate (and its schedule), batch size, number of epochs, weight
decay / regularization strength, and network width/depth. This session's
Advanced Optimization section already covered several of these in depth.
\end{itemize}
\end{vbframe}

\begin{vbframe}{Why Tuning Is Hard}
\begin{itemize}
\item The set of all configurations is the \textbf{hyperparameter space}
$\Lam$. It grows exponentially: two hyperparameters with 10 possible
values each already give $10 \cdot 10 = 100$ configurations to evaluate.
\item Evaluating a configuration $\lamv \in \Lam$ is a \textbf{black
box}: training a network and measuring validation performance is the only
way to know how good it is: there is no gradient or closed form to exploit.
\item Naive tuners search $\Lam$ directly:
\begin{itemize}
\item \textbf{Grid search}: evaluate a regular grid of configurations.
\item \textbf{Random search}: sample configurations at random. Often more
efficient than grid search at the same budget, since it explores more
distinct values per hyperparameter.
\end{itemize}
\end{itemize}
\begin{figure}
\centering
\includegraphics[width=11cm]{figure/i2ml-grid-random-search.png}
\caption{\footnotesize Grid search (left) vs. random search (right) over two hyperparameters. Source: SLDS LMU, I2ML.}
\end{figure}
\end{vbframe}

\begin{vbframe}{Grid Search vs. Random Search}
\begin{itemize}
\item Both are trivial to implement, support any parameter type, and
parallelize trivially.
\item \textbf{Grid search} discretizes each hyperparameter into a fixed set
of values and tries every combination. Downsides: scales combinatorially,
wastes evaluations on clearly bad regions, and the choice of grid points is
arbitrary.
\item \textbf{Random search} samples configurations uniformly at random
instead. It is an \emph{anytime} algorithm (stop whenever the budget runs
out) and never repeats the same value for a given hyperparameter twice.
\item \textbf{Key insight}: if only one of two hyperparameters actually
matters, a fixed evaluation budget gives random search far more distinct
values of the important one than grid search does. Grid search "wastes"
points repeating values of the irrelevant hyperparameter. This is a common
situation in practice, e.g. when one hyperparameter barely affects
performance.
\end{itemize}
\end{vbframe}
...
\begin{itemize}
\item Grid and random search never use past evaluations to guide future
ones. \textbf{Bayesian Optimization (BO)} does, by iterating:
\begin{enumerate}
\item Fit a \textbf{surrogate model} $\hat{c}(\lamv)$ (e.g. a Gaussian
Process) to the configurations evaluated so far, with an uncertainty
estimate $\hat{\sigma}(\lamv)$.
\item Propose the next configuration by maximizing an \textbf{acquisition
function} that trades off exploring uncertain regions against exploiting
regions the surrogate predicts are good (e.g. expected improvement).
\item Evaluate the proposed configuration, add it to the archive, repeat.
\end{enumerate}
\item Because the surrogate guides the search toward promising regions, BO
typically needs far fewer evaluations than grid or random search to reach a
good configuration, which matters since each evaluation means training a
network.
\end{itemize}
\begin{figure}
\centering
\includegraphics[width=9cm]{figure/i2ml-bo-surrogate.png}
\caption{\footnotesize Surrogate model $\hat{c}(\lamv)$ with uncertainty band (blue) approximating the true cost $c(\lamv)$ (dashed), and the resulting acquisition function $u(\lamv)$ (orange) used to pick the next point $\lamv^+$. Source: SLDS LMU, I2ML.}
\end{figure}
\end{vbframe}

\begin{vbframe}{Multi-Fidelity Optimization}
\begin{itemize}
\item Training a network to full convergence just to evaluate one
configuration is expensive. \textbf{Multi-fidelity} methods instead train
with a cheap approximation first (e.g. fewer epochs or a data subset; the
\textbf{fidelity}) and only spend the full budget on promising candidates.
\item \textbf{Successive Halving}: train all candidate configurations with
a small fraction of the full budget, discard the worse half, double the
budget for survivors, and repeat until one configuration (or the full
budget) remains.
\item \textbf{Hyperband} fixes Successive Halving's main weakness: a
genuinely good configuration can still be killed off too early depending on
the starting budget. It does this by repeating Successive Halving with several
different starting budgets (\enquote{brackets}) and aggregating.
\item Directly relevant to DL: early-stopping bad hyperparameter
configurations after a few epochs (rather than training every candidate to
convergence) is exactly this idea.
\end{itemize}
\begin{figure}
\centering
\includegraphics[width=8cm]{figure/i2ml-successive-halving.png}
\caption{\footnotesize Successive Halving: 8 configurations $\lamv^{(1)},\ldots,\lamv^{(8)}$ are raced down to the best as the budget fraction increases from 12\% to 100\%. Source: SLDS LMU, I2ML.}
\end{figure}
\end{vbframe}

\begin{vbframe}{The Untouched-Test-Set Principle}
\begin{itemize}
\item Once tuning has found a configuration $\lamv^*$, how well does it
actually perform? We cannot reuse the data used for tuning to also estimate
performance, since that would give an optimistically biased estimate.
\item To get an unbiased estimate, an \textbf{untouched test set} is
needed: the data used to select $\lamv^*$ (train/validation split) must
be kept separate from the test set used only once, at the very end, to
report performance.
\item \textbf{Nested resampling} generalizes this to reduce the variance of
the performance estimate: an outer resampling loop estimates generalization
performance, while an inner resampling loop (on only the outer-training
data) is used to tune $\lamv$.
\end{itemize}
\begin{figure}
\centering
\includegraphics[width=8cm]{figure/i2ml-nested-resampling.png}
\caption{\footnotesize Nested resampling: an inner loop tunes hyperparameters, an outer loop estimates performance of the tuned model. Source: SLDS LMU, I2ML.}
\end{figure}
\end{vbframe}
...
\footnotesize{
\begin{thebibliography}{99}
\bibitem[(SLDS LMU)]{hp1} SLDS LMU. \textit{Introduction to Machine Learning
(I2ML)}, chapter \enquote{Tuning \& Nested Resampling: In a Nutshell}.
\url{https://slds-lmu.github.io/i2ml/chapters/09_tuning/09-00-nutshell-tuning/}
\bibitem[(SLDS LMU)]{hp2} SLDS LMU. \textit{Introduction to Machine Learning
(I2ML)}, chapter \enquote{Hyperparameter Tuning: Basic Techniques}.
\url{https://slds-lmu.github.io/i2ml/chapters/09_tuning/09-03-basicalgos/}
\bibitem[(SLDS LMU)]{hp3} SLDS LMU. \textit{Introduction to Machine Learning
(I2ML)}, chapter \enquote{Hyperparameter Tuning: Advanced Tuning Techniques}.
\url{https://slds-lmu.github.io/i2ml/chapters/09_tuning/09-04-tuning-advanced/}
\end{thebibliography}
}
\end{vbframe}
...
\begin{vbframe}{Neural Architecture Search: Motivation}
\begin{itemize}
\item Manually designing state-of-the-art architectures (ResNet, Inception,
...) is time-consuming: complex architectures are the result of years of
trial and error by experts.
\item \textbf{Neural Architecture Search (NAS)} automates this: the goal is
to automatically find neural architectures with strong performance,
optionally subject to a resource constraint.
\item A decade-old idea, but mainstream only since 2017; initially
extremely expensive (hundreds of GPUs for weeks), but by now several
methods promise low overhead over training a single model.
\end{itemize}
\end{vbframe}

\begin{vbframe}{NAS Components and Definition}
\begin{itemize}
\item NAS methods are described by three components: a \textbf{search
space} $\mathcal{A}$ of candidate architectures, a \textbf{search strategy}
that picks which architecture $a \in \mathcal{A}$ to try next, and a
\textbf{performance estimation strategy} that estimates how good $a$ is.
\end{itemize}
\begin{figure}
\centering
\includegraphics[width=10cm]{figure/nas-components.png}
\caption{\footnotesize The NAS loop: search strategy proposes an architecture, performance estimation strategy evaluates it. Source: Lindauer \& Eggensperger, AutoML.org (ESSAI 2023).}
\end{figure}
\begin{itemize}
\item Formally: given a search space $\mathcal{A}$, dataset $D$, training
pipeline $P$, and a time/compute budget $t$, find $a \in \mathcal{A}$ within
budget $t$ with the highest validation accuracy when trained on $D$ with $P$:
$$\min_{a \in \mathcal{A}} \mathcal{L}_{\text{val}}(w^*(a), a) \quad \text{s.t.} \quad w^*(a) \in \argmin_w \mathcal{L}_{\text{train}}(w, a).$$
\end{itemize}
\end{vbframe}

\begin{vbframe}{One Example: DARTS (Differentiable Architecture Search)}
\begin{itemize}
\item Search strategies range from evolutionary algorithms and
reinforcement learning to Bayesian optimization, all expensive, since
naively each candidate architecture needs to be trained to be evaluated.
\item \textbf{DARTS} (Liu et al. 2018) is one way to make this affordable:
it relaxes the discrete choice of operation on each edge into a continuous,
differentiable weight, so the architecture itself can be optimized with
gradient descent instead of a separate search loop, with no RL or evolution
needed.
\end{itemize}
\begin{figure}
\centering
\includegraphics[width=11cm]{figure/nas-darts-pipeline.png}
\caption{\footnotesize DARTS pipeline: (1) randomly initialize architecture weights $\alpha$, (2) jointly optimize $\alpha$ and network weights $w$, (3) discretize by taking the arg max operation per edge, (4) retrain the resulting fixed architecture from scratch. Source: Lindauer \& Eggensperger, AutoML.org (ESSAI 2023).}
\end{figure}
\begin{itemize}
\item This \enquote{one-shot, differentiable} trick is one instance of a
broader NAS toolbox (ENAS, evolutionary search, Bayesian optimization,
zero-cost proxies, ...): the point isn't the specific algorithm, but that
architecture design itself can be automated and made tractable.
\end{itemize}
\end{vbframe}

\begin{vbframe}{NAS: Final Thoughts}
\begin{itemize}
\item Good architectures via NAS are feasible within a few hours today,
down from hundreds of GPU-hours for the earliest methods.
\item Gradient-based methods (DARTS and successors) are fastest but least
stable; Bayesian optimization with the right tricks remains highly
competitive.
\item NAS is still limited in what it can discover: it is unclear whether
NAS could have found genuinely novel architectures like the Transformer,
and NAS is not easily applicable to very expensive models (e.g. LLMs),
where even one full training run is already costly.
\end{itemize}
\end{vbframe}
...
\footnotesize{
\begin{thebibliography}{99}
\bibitem[(Lindauer \& Eggensperger, 2023)]{nas1} Lindauer, M., \&
Eggensperger, K. (2023). \textit{NAS: Neural Architecture Search}. ESSAI
SummerSchool 2023, AutoML.org.
\url{https://www.automl.org/wp-content/uploads/2023/07/3_ESSAI_NAS-1.pdf}
\end{thebibliography}
}
\end{vbframe}
...
\begin{itemize}
\item Momentum, Learning Rates, Adaptive Optimizers, and Batch Normalization
...
\item Hyperparameter Tuning assembled from \textbf{Introduction to Machine
Learning (I2ML)}, SLDS, LMU Munich. \url{https://github.com/slds-lmu/i2ml}
--- licensed \textbf{MIT}.
\item Neural Architecture Search adapted (and substantially condensed) from
Lindauer \& Eggensperger, \textit{NAS: Neural Architecture Search}, ESSAI
SummerSchool 2023, AutoML.org --- licensed \textbf{CC BY-SA}. This section
of the deck, including its two figures, is shared under the same
Attribution-ShareAlike terms: \url{https://creativecommons.org/licenses/by-sa/4.0/}
\item Content in this deck has been merged, reordered, and (for NAS)

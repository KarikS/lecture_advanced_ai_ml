...
\begin{vbframe}{Winter 1: Linear Models Hit Their Ceiling}
\begin{itemize}
\item \textbf{What was possible (1940s--1950s):} artificial neurons that combine weighted inputs and fire past a threshold; models whose weights could be \textbf{learned from data} rather than hand-set; these could learn \textbf{linear} decision boundaries.
\vspace{5mm}
\item \textbf{What wasn't possible:} no model in this class could learn a function that isn't linearly separable --- the canonical failure case is the XOR problem (we return to this in detail later). This was proven mathematically, not just observed empirically.
\vspace{5mm}
\item \textbf{Result (1969):} funding collapses, research stalls --- the first \enquote{AI Winter}.
\end{itemize}
\begin{figure}
\centering
...
\begin{vbframe}{Winter 2: Depth Without Traction}
\begin{itemize}
\item \textbf{What was possible (1960s--1980s):} weight-update rules based on the gradient of the error; learning algorithms for \textbf{deep, multilayer} networks; efficient gradient computation through many layers at once (backpropagation) --- in principle, non-linear decision boundaries were now learnable.
\vspace{5mm}
\item \textbf{What wasn't possible:} this didn't translate into practical superiority. Kernel machines and graphical models matched or beat these networks on many real tasks, and modeling long sequences remained a fundamental difficulty.
\vspace{5mm}
\item \textbf{Result (mid-1980s):} expectations vastly outpaced results, \enquote{AI} became associated with pseudoscience, and investment collapsed again --- the second \enquote{AI Winter}.
\end{itemize}
\begin{figure}
\centering
\includegraphics[width=6.5cm]{figure/ai_winter.jpg}
...
\begin{figure}
\centering
\includegraphics[width=5cm]{figure/rowcol.png}
\caption{\small Rows of $\Xmat$ are individual examples; columns of $\Wmat$ are individual (hidden) neurons.}
\end{figure}
\end{vbframe}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
